...
const cards = [
  ...
  normalizeCard({ id: "BU3", namePT: "Auto Burst", cardType: "magic", cost: 0, subtypes: ["burst"], effects: [{ id: "bu3-burst", type: "burst", timing: "afterLifeDecreases", operations: [{ type: "draw", count: 1 }] }] }),
  normalizeCard({ id: "MI1", namePT: "Mirage Test", cardType: "magic", cost: 2, mirage: { cost: 0, reduction: [] }, effects: [{ type: "mirage", timing: "mirage" }] }),
  normalizeCard({ id: "MI2", namePT: "Mirage Draw", cardType: "magic", cost: 2, mirage: { cost: 0, reduction: [] }, effects: [{ id: "mi2-set", type: "mirage", timing: "mirage", operations: [{ type: "draw", count: 1 }] }] }),
  normalizeCard({ id: "R3", namePT: "Double Symbol Spirit", cardType: "spirit", colors: ["red"], cost: 0, symbols: ["red", "red"], levels: [{ level: 1, cores: 1, bp: 3000 }] }),
  normalizeCard({ id: "TP1", namePT: "Turn Protection Source", cardType: "spirit", colors: ["white"], cost: 0, symbols: ["white"], levels: [{ level: 1, cores: 1, bp: 1000 }], abilities: [{ id: "tp1", event: "whenSummoned", actions: [{ type: "setTurnProtection", protection: { type: "limitSpiritAttackLifeDamage", maxDamage: 1 } }] }] }),
  normalizeCard({ id: "DB1", namePT: "Burst Discard Source", cardType: "spirit", colors: ["white"], cost: 0, symbols: ["white"], levels: [{ level: 1, cores: 1, bp: 1000 }], abilities: [{ id: "db1", event: "whenSummoned", actions: [{ type: "discardOpponentSetBurst" }] }] }),
...
test("v3.2.0 Mirage resolves structured on-set operations automatically", () => {
  let match = matchBase();
  match.phase = "main";
  match.players.player1.hand.unshift({ ...makePhysicalCard("MI2", index), instanceId: "mi2" });
  const deckBefore = match.players.player1.deck.length;
  const r = applyGameAction(match, { type: "SET_MIRAGE", instanceId: "mi2" }, "player1", index);
  assert.equal(r.ok, true);
  assert.equal(r.match.players.player1.mirage.cardId, "MI2");
  assert.equal(r.match.players.player1.deck.length, deckBefore - 1);
  assert.equal(r.manualResolutionNeeded, false);
});

test("v3.2.0 opens an automatic Burst window after Life decreases", () => {
  let match = matchBase();
  match.phase = "attack";
  match.players.player1.field.spirits = [fieldCard("R1", "burst-attacker")];
  match.players.player2.burst = { ...makePhysicalCard("BU3", index), instanceId: "auto-burst", faceDown: true };
  const deckBefore = match.players.player2.deck.length;

  let r = applyGameAction(match, { type: "DECLARE_ATTACK", instanceId: "burst-attacker" }, "player1", index);
  r = applyGameAction(r.match, { type: "PASS_FLASH" }, "player2", index);
  r = applyGameAction(r.match, { type: "PASS_FLASH" }, "player1", index);
  r = applyGameAction(r.match, { type: "DECLINE_BLOCK" }, "player2", index);
  r = applyGameAction(r.match, { type: "RESOLVE_BATTLE" }, "player1", index);
  ...
  assert.equal(r.ok, true);
  assert.equal(r.match.players.player2.life, 4);
  assert.equal(r.match.burstOpportunity?.playerId, "player2");

  const blocked = applyGameAction(r.match, { type: "ADVANCE_PHASE" }, "player1", index);
  assert.equal(blocked.ok, false);

  r = applyGameAction(r.match, { type: "ACTIVATE_BURST" }, "player2", index);
  assert.equal(r.ok, true);
  assert.equal(r.match.players.player2.burst, null);
  assert.equal(r.match.burstOpportunity, null);
  assert.equal(r.match.players.player2.deck.length, deckBefore - 1);
});

test("v3.2.0 Burst window can be passed without activating the set card", () => {
  let match = matchBase();
  match.phase = "attack";
  match.players.player1.field.spirits = [fieldCard("R1", "burst-pass-attacker")];
  match.players.player2.burst = { ...makePhysicalCard("BU3", index), instanceId: "pass-burst", faceDown: true };

  let r = applyGameAction(match, { type: "DECLARE_ATTACK", instanceId: "burst-pass-attacker" }, "player1", index);
  r = applyGameAction(r.match, { type: "PASS_FLASH" }, "player2", index);
  r = applyGameAction(r.match, { type: "PASS_FLASH" }, "player1", index);
  r = applyGameAction(r.match, { type: "DECLINE_BLOCK" }, "player2", index);
  r = applyGameAction(r.match, { type: "RESOLVE_BATTLE" }, "player1", index);
  ...
  r = applyGameAction(r.match, { type: "PASS_BURST" }, "player2", index);
  assert.equal(r.ok, true);
  assert.equal(r.match.burstOpportunity, null);
  assert.equal(r.match.players.player2.burst.cardId, "BU3");
});

test("v3.2.0 turn protection limits opposing Spirit attack Life damage", () => {
  let match = matchBase();
  match.players.player2.field.spirits = [fieldCard("TP1", "protection-source")];
  match = resolveCardEvent(match, { event: "whenSummoned", sourcePlayerId: "player2", sourceInstanceId: "protection-source" }, index).match;
  assert.equal(match.temporary.turnProtections.player2.limitSpiritAttackLifeDamage.maxDamage, 1);

  match.phase = "attack";
  match.players.player1.field.spirits = [fieldCard("R3", "double-attacker")];
  let r = applyGameAction(match, { type: "DECLARE_ATTACK", instanceId: "double-attacker" }, "player1", index);
  r = applyGameAction(r.match, { type: "PASS_FLASH" }, "player2", index);
  r = applyGameAction(r.match, { type: "PASS_FLASH" }, "player1", index);
  r = applyGameAction(r.match, { type: "DECLINE_BLOCK" }, "player2", index);
  r = applyGameAction(r.match, { type: "RESOLVE_BATTLE" }, "player1", index);
...
test("v3.2.0 Effect Engine can discard an opponent set Burst", () => {
  let match = matchBase();
  match.players.player1.field.spirits = [fieldCard("DB1", "discard-source")];
  match.players.player2.burst = { ...makePhysicalCard("BU1", index), instanceId: "burst-to-discard", faceDown: true };
  const trashBefore = match.players.player2.trash.length;
  const result = resolveCardEvent(match, { event: "whenSummoned", sourcePlayerId: "player1", sourceInstanceId: "discard-source" }, index);
  assert.equal(result.match.players.player2.burst, null);
  assert.equal(result.match.players.player2.trash.length, trashBefore + 1);
  assert.equal(result.match.players.player2.trash.at(-1).cardId, "BU1");
