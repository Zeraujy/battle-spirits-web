...
import { passBurstOpportunity } from "./burstRules.js";
...
export function applyGameAction(match, action, actorId, cardIndex) {
  ...
  if (
    ...
    !["ACTIVATE_BURST", "PASS_BURST"].includes(action.type)
  ) {
    return { ok: false, error: "Ative a Burst disponível ou passe a janela de Burst antes de continuar." };
  }
  ...
  switch (action.type) {
    ...
    case "PASS_BURST": result = passBurstOpportunity(match, actorId); break;
  ...
  const finished = finishResult(result, cardIndex, action.type);
  if (!finished?.ok || !finished.match) return finished;

  const withStructuredLog = appendStructuredAction(match, finished.match, action, actorId);
  return {
    ...finished,
    match: withStructuredLog,
    integrity: validateMatchState(withStructuredLog, cardIndex)
  };
}
