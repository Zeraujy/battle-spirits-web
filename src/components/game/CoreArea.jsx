...
export function CoreToken({ coreType="regular", source, canDrag=true, style, onDragStart, onClick, title }) {
  return <div
    className={`core-token ${coreType === "soul" ? "soul-core" : "regular-core"} ${onClick && canDrag ? "core-clickable" : ""}`}
    draggable={canDrag}
    style={style}
    ...
    onClick={(e)=>{
      e.stopPropagation();
      if (!canDrag || !onClick) return;
      onClick({ ...source, coreType });
    }}
...
export default function CoreArea({ title, playerId, zone, regularCount=0, soul=false, canControl=false, onCoreDrop, onCoreClick, accent="reserve", guidance=null }) {
  ...
  return <div className={`core-area ${accent} ${guidance ? `core-guidance core-guidance-${guidance.role || "active"}` : ""}`} onDragOver={(e)=>e.preventDefault()} onDrop={drop}>
    <div className="core-area-header">
      <strong>{title}</strong>
      <span>{regularCount}{soul ? " + Soul" : ""}</span>
    </div>
    {guidance && (
      <div className="core-area-guidance" aria-live="polite">
        <b>{guidance.label}</b>
        {guidance.detail && <small>{guidance.detail}</small>}
      </div>
    )}
    <div className="core-area-field">
      {tokens.map((index)=>{
        const pos = positions[index] || seedPosition(index, zone === "trash" ? 4 : 1);
        return <CoreToken key={`r-${index}`} coreType="regular" canDrag={canControl} source={{ playerId, zone, tokenIndex:index }} onDragStart={dragStart} onClick={onCoreClick} style={{ left:`${pos.x}%`, top:`${pos.y}%` }} />;
      })}
      {soul && <CoreToken coreType="soul" canDrag={canControl} source={{ playerId, zone, tokenIndex:"soul" }} onDragStart={dragStart} onClick={onCoreClick} style={{ left:`${soulPos.x}%`, top:`${soulPos.y}%` }} />}
